...
def anythingllm_query(message: str):
    """
    向 AnythingLLM 知识库提问，获取文档仓库中的信息。
    当用户提到“文档仓库”、“文件仓库”、“仓库”或需要从知识库获取信息时调用。
    """
...
def list_available_skills():
    """读取项目目录下.agents/skills文件夹里所有一级子文件夹中的SKILL.md文件，从YAML front matter中提取name和description字段。"""
    ...
    for skill_folder in os.listdir(skills_dir):
        ...
        if os.path.exists(skill_path):
            try:
                ...
                print(f"解析技能 {skill_folder} 失败: {str(e)}")
    return available_skills

def load_skill_content(name: str):
    """加载SKILL.md文件中YAML front matter之后的正文内容。"""
    skills_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), '.agents', 'skills')
    # 假设技能文件夹名与 metadata 中的 name 一致，或者我们可以遍历寻找
    for skill_folder in os.listdir(skills_dir):
        skill_path = os.path.join(skills_dir, skill_folder, 'SKILL.md')
        if os.path.exists(skill_path):
            try:
...
TOOLS_SCHEMA = [
    {
        ...
        "function": {
            ...
            "description": "向 AnythingLLM 知识库提问，获取文档仓库中的信息。当用户提到“文档仓库”、“文件仓库”、“仓库”或需要从知识库获取信息时调用。",
            "parameters": {
                "type": "object",
                "properties": {
                    "message": {"type": "string", "description": "用户的问题，用于查询 AnythingLLM 知识库"}
                },
                "required": ["message"]
            }
        }
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "当 LLM 判断需要使用某个技能时，调用此工具加载技能的详细正文说明。你需要提供技能的名称。",
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {"type": "string", "description": "技能名称 (如 'notice')"}
                },
                "required": ["name"]
            }
        }
...
def chat_with_summary():
    ...
    messages = [
        {
            "role": "system", 
            "content": f"你是一个具备文件系统操作、实时网页访问和 AnythingLLM 知识库查询能力的 AI 助手。今天的日期是 {current_date}。你可以通过调用工具来执行本地文件读写、目录管理、通过 URL 抓取网页内容，或者向 AnythingLLM 知识库提问。当用户要求你查看某个网址或搜索信息时，请务必使用 fetch_web_content 工具。当用户提到“文档仓库”、“文件仓库”、“仓库”或需要从知识库获取信息时，请优先使用 anythingllm_query 工具。\n\n此外，你还拥有以下可选技能：\n{skills_json}\n\n如果你认为需要使用其中的某个技能，请先调用 load_skill_content 工具获取该技能的详细执行指令，然后严格按照指令执行。"
        }
    ]
